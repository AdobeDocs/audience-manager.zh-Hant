...
|---|---|---|---|
| [!DNL AAM UUID] | [!DNL Adobe Audience Manager Unique User ID]，也稱為[!UICONTROL Device ID]。 [!DNL Audience Manager]與其互動的每個裝置相關聯的數字38位數裝置ID。 每當您在[!DNL Audience Manager] UI中看到提及不重複使用者時，請思考此ID。 Audience Manager會將此ID儲存為[!DNL cookie]第三方網域中的`demdex.net`。 | 在[!DNL DCS]個呼叫中，`uuid`的前置詞為`d_`首碼。 <br>範例: `d_uuid = 07955261652886032950143702505894272138` | 建立[!DNL traits]相似模型[!UICONTROL Device ID]和[建立區段](../features/algorithmic-models/create-model.md)時，您可以依[篩選](../features/segments/segment-builder.md)。 執行[!UICONTROL Device ID]特徵的一般報表[和](../reporting/general-reports.md)特徵的趨勢報表[時，您也可以依](../reporting/trend-reports.md)篩選結果。 |
| [!DNL ImsOrgId] | [!DNL Organization ID]。這是公司註冊[!DNL Experience Cloud]帳戶時所提供的ID。 | `5DC5123F5245B1D20A490D46@AdobeOrg` | 在[!DNL Audience Manager]使用者介面中不可見。 若要瞭解如何尋找您公司的[!DNL Organization ID]，請閱讀[尋找組織ID](https://experienceleague.adobe.com/docs/core-services/interface/manage-users-and-products/organizations.html?lang=zh-Hant#concept_EA8AEE5B02CF46ACBDAD6A8508646255)。 |
...
| [!DNL ECID]，[!DNL MID] | [!DNL Experience Cloud] ID。 [!DNL Experience Cloud] ID （[!DNL ECID]，舊式縮寫[!DNL MID]或[!DNL MCID]）是從您的[!DNL Organization ID]和[!DNL Audience Manager] [!UICONTROL Unique User ID]以數學方式所衍生。 只要這些ID保持不變，為特定使用者產生正確的[!DNL ECID]就只是數學問題。 使用相同的[!DNL Organization ID]和[!DNL Audience Manager] [!DNL UUID]，您每次都會獲得相同的[!DNL ECID]值。 您可以在[!DNL ECID]Cookie和Experience Cloud ID[檔案中深入閱讀](https://experienceleague.adobe.com/docs/id-service/using/intro/cookies.html?lang=zh-Hant#section-15f69c0bac394b4b9966a23fbc586d17)。 | `mid = 08382830887934830189014177072406221371` | 在[!DNL Audience Manager]使用者介面中不可見。 |
| [!DNL SID] | [!UICONTROL Trait ID]。[!UICONTROL Trait ID]在[!DNL traits]環境中唯一識別[!DNL Audience Manager]。 | 在[!DNL DCS]個呼叫中，`SID`的前置詞為`d_`首碼。 <br>範例`d_sid=289983`。 | [!UICONTROL Trait ID]會指派給每個[!DNL trait]，並顯示在[特徵](../features/traits/trait-details-page.md)頁面的使用者介面中。 |
| [!DNL SID] | [!UICONTROL Segment ID]。[!UICONTROL Segment ID]在[!DNL segments]環境中唯一識別[!DNL Audience Manager]。 | 在[!DNL DCS]個呼叫中，`SID`的前置詞為`d_`首碼。 <br>範例`d_sid=4798574`。 | [!UICONTROL Segment ID]已指派給每個[!DNL segment]，並顯示在[區段](../features/segments/segment-summary-view.md)頁面的使用者介面中。 |
...
| [!DNL destID] | [!UICONTROL Destination ID]。[!UICONTROL Destination ID]在[!DNL destinations]環境中唯一識別[!DNL Audience Manager]。 使用者介面中的每個[!DNL destination]都會指派一個ID。 | `2523` | [!UICONTROL Destination ID]已指派給每個[!DNL destination]，並顯示在[目的地](../features/destinations/destinations-home.md)頁面的使用者介面中。 |
...
| [!DNL DPUUID] | [!UICONTROL Data Provider Unique User ID]，也稱為[!DNL CRM ID]或[!UICONTROL Cross-Device ID]。 協力廠商ID。 這是識別您自己[!DNL CRM]系統中一般使用者的識別碼。 您可以將[!DNL DPUUIDs]與[!DNL Audience Manager] [!DNL UUIDs]同步，也可以在ID同步處理中，從您不同的[!DNL DPUUIDs] ([!UICONTROL Data Sources])同步[!DNL DPIDs]。 | 在[!DNL DCS]個呼叫中，`dpuuid`的前置詞為`d_`首碼。 <br>範例`d_dpuuid=2132-3423vn-343fds-3432r`。 | 建立[!DNL traits]相似模型[!UICONTROL Cross-Device ID]和[建立區段](../features/algorithmic-models/create-model.md)時，您可以依[篩選](../features/segments/segment-builder.md)。 執行[!UICONTROL Cross-Device ID]特徵的一般報表[和](../reporting/general-reports.md)特徵的趨勢報表[時，您也可以依](../reporting/trend-reports.md)篩選結果。 |

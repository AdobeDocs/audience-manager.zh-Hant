...
| 主題 | 說明 |
|--- |----|
| [Audience Manager API](/help/using/api/rest-api-main/rest-api-main.md) | 我們正在將產品檔案中的所有API[移轉至Swagger API。 ](/help/using/api/api-swagger-migration.md)我們移轉過的最新兩個API為： <ul><li>[演演算法模型API](https://bank.demdex.com/portal/swagger/index.html#/Algorithmic_Models_API)</li><li>[衍生訊號API](https://bank.demdex.com/portal/swagger/index.html#/Derived_Signals_API)</li></ul> |
...
| [自訂合作夥伴整合](/help/using/integration/sending-audience-data/custom-partner-integrations.md) | 本頁面列出Audience Manager與資料合作夥伴之間的自訂整合。 <br> Oracle Data Cloud - Audience Manager會透過傳入資料檔案，從Audience Marketplace的Oracle Data Cloud擷取Cookie和行動ID資料。 本頁面所述的自訂整合規格僅指包含行動 ID (IDFA 和 Android 裝置 ID) 的傳入資料檔案。 |
...
| [適用於 IAB TCF 的 Audience Manager 外掛程式](/help/using/overview/data-security-and-privacy/aam-iab-plugin.md) | 您對使用者的隱私義務中，其中一個重要面向是取得並傳達使用者對其個人資料使用方式（即「目的」）和使用者（即「公司」）所做的選擇。 Adobe可讓您透過[選擇加入功能](https://experienceleague.adobe.com/docs/id-service/using/implementation/opt-in-service/optin-overview.html)和[IAB透明與同意架構(TCF)](https://iabtechlab.com/standards/gdpr-transparency-and-consent-framework/)支援，管理使用者的隱私權選擇，並與使用者針對該選擇溝通。 <br> <br>  本文說明支援IAB TCF的Audience Manager使用案例，以及如何在Audience Manager中實作IAB TCF支援。 |

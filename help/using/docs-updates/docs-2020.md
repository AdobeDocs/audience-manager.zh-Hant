...
| 主題 | 說明 |
|----|----|
| [預測受眾](/help/using/features/algorithmic-models/predictive-audiences.md) | <ul><li>**預測性觀眾模型仿製**:您現在可以複製現有機型，並根據需求變更其配置。 閱讀「 [仿製和編輯預測性受眾模型](/help/using/features/algorithmic-models/predictive-audiences.md#clone-predictive-audiences) 」以取得詳細資訊。</li><li>**預測性對象批次分類**:除了以不同角色即時分類訪客外，預測性觀眾現在還支援批次分類，協助您將已登入的使用者分類並啟動至批次目的地。</li></ul> |
| [重疊報表](/help/using/reporting/dynamic-reports/dynamic-reports.md) | 我們已更新用於計算「重疊報表」([特徵對特徵](/help/using/reporting/dynamic-reports/trait-trait-overlap-report.md)、 [區段對特徵](/help/using/reporting/dynamic-reports/segment-trait-overlap-report.md)、 [區段對區段](/help/using/reporting/dynamic-reports/segment-segment-overlap-report.md))的方法。 「重疊報表」計算現在是以報表唯一值和MinHash簽名(而非 [1/54資料取樣](/help/using/reporting/report-sampling.md#data-sampling-ratio))為基礎。 |
...
| [傳入的S3檔案名稱](/help/using/integration/sending-audience-data/batch-data-transfer-explained/inbound-s3-filenames.md) 和傳 [入的FTP檔案名稱](/help/using/integration/sending-audience-data/batch-data-transfer-explained/inbound-ftp-filenames.md) | 我們更新了檔案名稱語法中的定 `DPID` 義 `DPID_TARGET_DATA_OWNER` 和欄位範例。 |

## August, 2020 {#aam-august-updates}

| 主題 | 說明 |
|---- |----|
| [Google客戶符合人員型目標](../features/destinations/people-based-destinations-prerequisites.md#gcm) | 您現在可以透過 [!UICONTROL People-Based Destinations] 以下方式來定位使用者 [!DNL Google Customer Match]: 您必須先將 [!UICONTROL People-Based Destinations] 您新增至其允許清單，才能使用將第一方 [!DNL Google Customer Match] 對象區 [!DNL Google] 段傳送至目的地。 如需詳細 [資訊，請參閱以人為本的目標](../features/destinations/people-based-destinations-prerequisites.md#gcm) 。 |
| [新的SFTP伺服器](../faq/faq-data-collection.md) | 您現在可以使用新 [!DNL SFTP] 的伺服[!DNL ftp-in-gtw] 器( [!DNL ftp-out-gtw]和), [!DNL AWS SFTP Gateway] 而 [!DNL S3]且 |

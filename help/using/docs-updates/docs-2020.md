...
| 主題 | 說明 |
|----|----|
| [預測受眾](/help/using/features/algorithmic-models/predictive-audiences.md) | <ul><li>**預測性觀眾模型仿製**:您現在可以複製現有機型，並根據需求變更其配置。如需詳細資訊，請閱讀[仿製和編輯預測性對象模型](/help/using/features/algorithmic-models/predictive-audiences.md#clone-predictive-audiences)。</li><li>**預測性對象批次分類**:除了以不同角色即時分類訪客外，預測性觀眾現在還支援批次分類，協助您將已登入的使用者分類並啟動至批次目的地。</li></ul> |
| [重疊報表](/help/using/reporting/dynamic-reports/dynamic-reports.md) | 我們已更新用於計算重疊報表的方法([Trait-to-Trait](/help/using/reporting/dynamic-reports/trait-trait-overlap-report.md)、[Segment-to-Trait](/help/using/reporting/dynamic-reports/segment-trait-overlap-report.md)、[Segment-to-Segment](/help/using/reporting/dynamic-reports/segment-segment-overlap-report.md))。 「重疊報表」計算現在是以報表唯一值和MinHash簽名（而非[1/54資料取樣](/help/using/reporting/report-sampling.md#data-sampling-ratio)）為基礎。 |
...
| 主題 | 說明 |
|---- |----|
| [Audience Manager中的協助功能](/help/using/overview/accessibility-in-aam.md) | 協助功能是指讓軟體產品可供使用的一系列功能，讓視覺、聽覺、認知、運動或其他殘障人士盡可能少的努力。 為了讓Audience Manager更容易為每個人使用，我們開發了多種協助工具功能支援，詳細說明請參閱本文。 |
| [入站S3檔](/help/using/integration/sending-audience-data/batch-data-transfer-explained/inbound-s3-filenames.md) 案和 [入站FTP檔案](/help/using/integration/sending-audience-data/batch-data-transfer-explained/inbound-ftp-filenames.md) | 我們更新了檔案名語法中`DPID`和`DPID_TARGET_DATA_OWNER`欄位的定義和示例。 |

## 2020年8月{#aam-august-updates}

| 主題 | 說明 |
|---- |----|
| [Google客戶符合人員型目標](../features/destinations/people-based-destinations-prerequisites.md#gcm) | 您現在可以透過[!DNL Google Customer Match]使用[!UICONTROL People-Based Destinations]來定位使用者。 您必須先將[!DNL Google]新增至您的允許清單，才能使用[!UICONTROL People-Based Destinations]將您的第一方對象區段傳送至[!DNL Google Customer Match]目的地。 如需詳細資訊，請參閱[以人為本的目標](../features/destinations/people-based-destinations-prerequisites.md#gcm)檔案。 |
| [新的SFTP伺服器](../faq/faq-data-collection.md) | 您現在可以使用新的[!DNL SFTP]伺服器（[!DNL ftp-in-gtw]和[!DNL ftp-out-gtw]），這些伺服器使用[!DNL AWS SFTP Gateway]而非[!DNL S3]。 |
...
| 主題 | 說明 |
|---- |----|
| [Audience Manager 與 Adobe Experience Platform 之間的受眾共用](/help/using/integration/integration-aep/aam-aep-audience-sharing.md#aep-aam-segment-population-differences) | 新章節&#x200B;**瞭解Audience Manager和Experience Platform之間的區段人口差異**&#x200B;說明Experience Platform和Audience Manager之間人口數可能不同的原因。 |
| [Audience Manager REST API快速入門](/help/using/api/rest-api-main/aam-api-getting-started.md#jwt) | 我們新增了一個新區段&#x200B;**JWT（服務帳戶）驗證，使用[!DNL Adobe I/O]**。 請參閱本節，以使用[!DNL Adobe I/O]驗證Audience Manager API。 |

...
* 已完成或 [!DNL CRM] 資料倉庫擷取計畫；
* 定義受眾細分需求；
* 完整的資料分類；
* 已開發的協力廠商資料整合計畫。

## 建立、測試和培訓階段 {#build-test-train-phase}

在構建、測試和培訓階段，您將與指定的合作夥伴解決方案主管一起審查資料收集策略和原型。

<!-- c_build_test_train.xml -->

您的資料收集策略將會經過端對端測 [!DNL QA] 試。 合作夥伴解決方案將追蹤發現的錯誤，並與我們的系統工程師協調問題解決。 客戶培訓可以與這些其他工作同時開始。
...
在啟動、支援和最佳化階段，您的資料收集和原型化實作會從開發階段移至即時的生產環境。 我們將繼續針對產品熟悉度和策略進行培訓，協助您透過資料導向 [!DNL ROI] 的最佳化來提升業績。
  ...
  <tr valign="top"> 
   ...
   <td colname="col2">為資料收集建立真實特徵和區段： 
    <ul id="ul_21C7E86A7AF749CD8ECBE129DE2C641A"> 
     <li id="li_4FCCC879B7A44752A55DEAC2D6434220">建立真實特徵和細分 </li> 
...
雖然部署程式看起來可能很複雜，但程式碼實作就像在網站的結尾標籤 [!DNL JavaScript] 旁加幾行 `</body>` 一樣簡單。

<!-- c_code.xml -->

### 部署工作

Audience manager程式碼片段會呼 [!DNL Akamai] 叫下載先前在使用者介面中設定的業務規則。 此外，用戶端瀏覽器會快取此資訊，以縮短頁面和伺服器載入時間。 我們的程式碼和資料收集方法旨在維護庫存的使用者體驗。
...
請在這裡 [聯絡我們](https://www.adobe.com/products/audiencemanager.html) ，以取得詳細資訊，並開始使用Audience Manager。
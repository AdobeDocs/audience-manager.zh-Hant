...
>[!NOTE]
>
>在桌子上， **粗臉** type標識每個宏及其相關輸出。 對於格式示例， `<` `>` 已添加符號，以幫助直觀地分隔每個宏。
...
<table id="table_408C6DD2B9D54550B003EAC93562E64F"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>請參閱下面的單獨部分。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p><b>格式:</b> </p> <p> 
     ...
     </code></p> <p> <p>注：在第一個示例中，宏僅返回 <code> SEGMENT_LIST </code> 因為 <code> REMOVED_SEGMENT_LIST </code> 為空。 第二個示例返回兩個宏的資料。 </p> </p> </td> 
  </tr> 
  <tr> 
...
幫助您瞭解 `DPUUID` 宏輸出資料，假設我們有2 `DPID`映射到 `DPUUID`如下所示：

* DPID `1111` 映射到DPUUID `AAAA` (時間戳= 1和 `BBBB` （時間戳= 2）。
* DPID `2222` 映射到DPUUID `CCCC`。

在這些情況下，下表列舉了一些可能的格式字串及其輸出。
...
<table id="table_6A6D94F994C1475BB09126BA0B815B1F"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p>返回單個DPID的所有映射 </p> </td> 
  ...
  <tr> 
   <td colname="col1"> <p>為所有DPID返回最大1個映射 </p> </td> 
   <td colname="col2"> <p> <code> &lt;DPUUIDS; format="dpids=1111,2222|maxMappings=1|format=json"&gt; </code> </p> </td> 
   <td colname="col3"> <p> <code> [["1111","BBBB"],["2222","CCCC"]] </code> </p> <p>對於DPID <code> 1111 </code>，宏映射到DPUUID <code> BBBB </code> 只是因為該ID的時間戳較大。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>為單個DPID返回最多2個映射 </p> </td> 
   <td colname="col2"> <p> <code> &lt;DPUUIDS; format="dpids=2222|maxMappings=2|format=json"&gt; </code> </p> </td> 
   <td colname="col3"> <p> <code> [["2222","CCCC"]] </code> </p> <p>即使 <code> maxMappings=2 </code>，此宏僅返回1個DPID到DPUUID映射，因為指定的DPID只有一個DPUUID。 </p> </td> 
  </tr> 
 </tbody> 
</table>

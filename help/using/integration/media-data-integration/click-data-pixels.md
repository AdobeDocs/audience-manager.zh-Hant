...
點按追蹤可讓您測量整個促銷活動中的訪客參與度，因為它會記錄第三方創意人員的點按式活動。 與[印象收集](/help/using/integration/media-data-integration/impression-data-pixels.md)類似，事件調用被發送到[!DNL Audience Manager]資料收集伺服器([!DNL DCS])進行處理。 接著，訪客會重新導向至預期的網址。

>[!NOTE]
>
>請連絡您的[!DNL Audience Manager]諮詢或客戶主管，以取得用戶端網域的確切[!DNL URL]資訊。

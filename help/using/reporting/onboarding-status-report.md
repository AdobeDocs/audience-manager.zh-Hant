...
>[!NOTE]
>
>只有具備管理員許可權的使用者才能在Audience Manager使用者介面中看到此報表。 您可以將非管理員使用者的電子郵件新增至報表，以通知其已上傳傳入檔案的狀態。 請參閱[接收電子郵件通知](/help/using/reporting/onboarding-status-report.md#receive-email-notifications)。
...
<table id="table_1D44A2E6B4C847848B818190DD336841"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>列出<span class="keyword"> Audience Manager</span>從您選取的傳入資料來源接收及處理的檔案。 </p> <p>如果檔案名稱格式不正確，檔案處理將會失敗。 檔案名稱要求依您傳送此資料至<span class="keyword"> Audience Manager</span>的方式而異。 傳遞方式包括<span class="keyword"> Amazon S3</span>和FTP。 如需如何命名檔案的說明，請參閱： </p> <p> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>列出格式錯誤的<span class="keyword">個Audience Manager</span>使用者ID (UUID)數目。 這通常代表ID： </p> 
...
<tr> 
   ...
   <td colname="col2"> <p>列出格式不正確的全域裝置ID數目。 如需裝置ID的格式設定方式以及您應根據裝置型別使用何種全域資料來源的詳細資訊，請參閱Audience Manager<a href="../reference/ids-in-aam.md">中的</a>ID索引和<a href="../features/global-data-sources.md">全域資料來源</a>。</p>
...
<tr> 
   ...
   <td colname="col2"> <p>這些已上線的ID <span class="keyword"> Audience Manager</span>無法與現有的ID相符。 當<span class="keyword"> Audience Manager</span>尚未執行ID同步時，已上線的ID可以具有此狀態，或即使同步後仍無法與ID相符。 </p> <p>如果行動識別碼不相符，<span class="keyword"> Audience Manager</span>將： </p> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>列出<span class="keyword"> Audience Manager</span>無法比對已上線特徵的特徵。 這可能是下列原因所致： </p> 
    <ul id="ul_43619035AB6641B6949302FB50BDB5B1"> 
     ...
     <li id="li_A1C708A007D24EE09B7C629AFC6E43C3"><span class="keyword"> Audience Manager</span>中尚未定義的特徵。 </li> 
    </ul> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>已成功儲存的記錄數。 由於檔案格式錯誤，<span class="keyword"> Audience Manager</span>可能無法儲存所收到的部分記錄。 儲存的記錄數可以少於接收的記錄數。 </p> </td> 
  </tr> 
  <tr> 

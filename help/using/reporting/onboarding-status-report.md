...
<table id="table_1D44A2E6B4C847848B818190DD336841"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>列出從您選取的傳入資料來源接收並處理<span class="keyword"> Audience Manager</span>的檔案。 </p> <p>如果檔案名的格式不正確，檔案處理將失敗。 檔案名稱需求會依您傳送此資料至<span class="keyword"> Audience Manager</span>的方式而異。 傳送方法包括<span class="keyword"> Amazon S3</span>和FTP。 如需如何命名檔案的指示，請參閱： </p> <p> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>列出格式不正確的<span class="keyword"> Audience Manager</span>使用者ID(UUID)數。 通常，這表示ID: </p> 
...
<tr> 
   ...
   <td colname="col2"> <p>列出格式不正確的全域裝置ID的數目。 請參閱Audience Manager</a>和<a href="../features/global-data-sources.md">全域資料來源</a>中的<a href="../reference/ids-in-aam.md">ID索引，以取得裝置ID應如何格式化以及您應根據裝置類型使用哪些全域資料來源的詳細資訊。</a></p>
...
<tr> 
   ...
   <td colname="col2"> <p>這些是已登入的ID <span class="keyword"> Audience Manager</span>無法與現有ID相符。 當<span class="keyword"> Audience Manager</span>尚未執行ID同步，或者即使在同步後仍無法比對ID時，已登入的ID可能會有此狀態。 </p> <p>如果行動ID不相符，<span class="keyword"> Audience Manager</span>將： </p> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>成功儲存的記錄數。 由於檔案格式錯誤，所以<span class="keyword"> Audience Manager</span>可能無法儲存收到的部分記錄。 儲存的記錄數可以小於接收的記錄數。 </p> </td> 
  </tr> 
  <tr> 

...
1. 提供整合程式碼。 整合程式碼是您專屬的唯一ID，用於此資料來源。
1. 在清單 **[!UICONTROL ID Type]** 中，選擇 **[!UICONTROL Cross Device]**。
1. 在清單 **[!UICONTROL ID Definition]** 中，選擇定義資料源類型的選項。 選項包括:
...
與這些設定關聯的文本欄位允許您使用「配置檔案合 [!UICONTROL Data Source] 並規則」選項中顯示的別名來 [更名](../../features/profile-merge-rules/merge-rule-definitions.md)。 例如，如果向中添加別名， **[!UICONTROL Use as Authenticated Profile]**&#x200B;該名稱將出現在列 [!UICONTROL Authenticated Profile Options] 表中。 如果向中添加別名， **[!UICONTROL Use as a Device Graph]**&#x200B;該名稱將出現在列 [!UICONTROL Device Options] 表中。
...
**** 必要條件：需要跨裝置資料來源才能建立 [!UICONTROL Profile Merge Rule]。 請參 [閱建立資料來源](../../features/manage-datasources.md#create-data-source)。

>[!TIP]
>
>如需這 [些不同控制項的說明](../../features/profile-merge-rules/merge-rule-definitions.md) ，請參閱描述檔合併規則選項定義。
...
[「資料匯出控制](../../features/data-export-controls.md) 」是可套用至您的選擇性分類規則 [!UICONTROL Profile Merge Rule]。 當該動作違反資料隱私或使用協定時，您無法將資料傳送至目的地。 如果您未使用，請略過本節 [!UICONTROL Data Export Controls]。
...
2. 選取 **[!UICONTROL Authenticated Profile Option]** （最多3個，最多）。 這些是您 [先前建立的跨裝置](../../features/profile-merge-rules/merge-rules-start.md) 資料來源。
...
使 [!UICONTROL Experience Cloud ID Service] 用時建議使用 [DIL](../../dil/dil-overview.md) 和最新版本 [!UICONTROL Profile Merge Rules]。 不過，您不必使用此 [!UICONTROL Experience Cloud ID Service] 功能即可。 如果您只是使用，請 [!UICONTROL DIL]參閱下 [方的舊版DIL](../../features/profile-merge-rules/merge-rules-start.md#legacy-dil) 。
...
最新版本 [!UICONTROL DIL] 現在會自動從中的 [!UICONTROL declared ID] 函 `visitorService` 數中取出(請 `DIL.create` 參閱 [Declared ID Variables](../../features/declared-ids.md#declared-id-variables))。 檢查函 `DIL.create` 數，以確定此設定正確，如下列程式碼範例所示。
...
如需詳細資訊，請參閱「宣告的ID [!UICONTROL DIL] 變數」中 [的舊版章節](../../features/declared-ids.md#declared-id-variables)。

...
[!UICONTROL Cross-Device Options]可讓您選取已驗證和未驗證的使用者，並將其跨裝置設定檔用於分段。 這些選項可協助您識別並觸及共用裝置上的特定使用者。 如需匿名與已驗證使用者的詳細資訊，請參閱Audience Manager](../../reference/visitor-authentication-states.md)中的[訪客驗證狀態。

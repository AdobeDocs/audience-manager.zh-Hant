...
您透過在[!UICONTROL Profile Merge Rule]中選取下述選項來建置[!UICONTROL Profile Merge Rule Setup]。
...
* 已針對先前符合資格的[!UICONTROL segment]執行批次[!UICONTROL traits]評估。
* 同時支援即時和批次[!UICONTROL Profile Merge Rules]評估的[!UICONTROL segment]將即時訪客活動與先前限定的[!UICONTROL traits]結合在一起。
...
[!UICONTROL Cross-Device Options]可讓您選取已驗證和未驗證的使用者，並將其跨裝置設定檔用於分段。 這些選項可協助您識別並觸及共用裝置上的特定使用者。 如需匿名與已驗證使用者的詳細資訊，請參閱Audience Manager中的[訪客驗證狀態](../../reference/visitor-authentication-states.md)。
...
<table id="table_4CE2DD312F54480E96BEAF72800789FB"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>在訪客已登入您的網站時，通知<span class="keyword"> Audience Manager</span>讀取及寫入資料至已驗證的設定檔。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <b><span class="uicontrol">上次驗證的設定檔</span></b> </p> </td> 
   <td colname="col2"> <p>告知<span class="keyword"> Audience Manager</span>從上次登入裝置之使用者的已驗證設定檔中讀取資料。 </p> <p>選取後，如果使用者為匿名，<span class="keyword"> Audience Manager</span>將不會將新特徵資料寫入已驗證的設定檔。 驗證後，新特徵資料會寫入使用者的已驗證設定檔中。 </p> </td>
  </tr> 
  <tr> 
...
[!UICONTROL Device Options]可讓您選取&#x200B;*`device profile`*&#x200B;所使用的[!UICONTROL Profile Merge Rule]型別。 裝置設定檔是從匿名瀏覽活動所收集的[!UICONTROL traits]所建置。 [!UICONTROL profile merge rule]至少包含[!UICONTROL authenticated option]和[!UICONTROL device option]。
...
<tr> 
   ...
   <td colname="col2"> <p>這些選項可讓您根據協力廠商提供的裝置圖表技術建立合併規則。 協力廠商裝置圖表提供： </p> <p> 
     ...
     </ul> </p> <p>若要使用這些選項，您必須是已與<span class="keyword"> Audience Manager</span>整合的裝置圖表提供者的客戶。 請聯絡您的客戶經理，以取得詳細資訊或開始使用。 </p> </td>
  </tr>
 </tbody>
</table>
...
根據在[!DNL Experience Cloud]外部定義的合併規則，從其他[!DNL Audience Manager]解決方案自動建立的對象區段會使用[!UICONTROL External Merge Policy]標示為。 例如，請參閱[在Audience Manager和Adobe Experience Platform之間共用對象](../../integration/integration-aep/aam-aep-audience-sharing.md)。

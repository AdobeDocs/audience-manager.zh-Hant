...
<table id="table_6DC02E219B074BF782EAA0E9DB9495FB"> 
 ...
 <tbody> 
   ...
   <td colname="col2"> 
    <draft-comment> 
     ...
     <ul id="ul_B609B402A29D44898DF0B1ABC6011D40"> 
      <li id="li_27A530DE2AEB48069BECFB2D78E94C4E">已在回顧視窗 <b>AND中實現規則型或已登入特徵</b> </li> 
      <li id="li_47C44ECAEC5844DEB11C6A93C8F061BB">不論同步的時間為何，都能與選擇的目標同步ID。 </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <b>客戶匹配率</b> </p> </td> 
   <td colname="col2"> <p>客戶可定址對象÷客戶總對象，表示為%。 </p> </td> 
  </tr>
   <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>在報表回顧期間屬於區段並在您的網站上同步作用中ID的使用者人數。 區段可以透過Audience Marketplace中取得的特性，包含您自己的第一方資料、第二方資料和第 <a href="../features/audience-marketplace/marketplace-data-buyers/marketplace-data-buyers.md"> 三方資料</a>。 </p> <p> <p>提示：當與1天回顧期間搭配使用時，此度量可協助您瞭解區段的目前狀態。 這是因為「區 <span class="wintitle"> 段可定址對象</span> 」量度代表在前一天一直留在區段中的使用者。 結合此點與 <span class="keyword"> Audience Manager每天重新整理</span><span class="wintitle"></span> 「可定址對象」的事實，結合此度量和回顧期間可提供區段的最新快照。 </p> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <b>區段比對率</b> </p> </td> 
   <td colname="col2"> <p>「分段可定址對象」÷以%表示的分段總人口。 </p> </td> 
  </tr>  
 </tbody> 
</table>
...
您可在「可定址對象」登陸頁面上看到的三個度量代表：

| 量度 | 說明 |
---------|----------|
| **可定址對象（裝置）** | 此量度代表過去30天的客戶可定址 *對象（如上表所述）。* |
| **比對率** | 此量度代表過去30天的「可定址對象符合率」(如上 *表所述)*。 |
| **終身可定址觀眾（裝置）** | 在報告回顧期間，平台層級與所有Audience manager客戶互動且可能與此目標相符的所有裝置計數。 如需 [詳細資訊，請參閱平台層](/help/using/features/addressable-audiences.md#platform-level-metrics) 級量度。 |
...
* **** 預測和規劃：資 [!UICONTROL Segment Addressable Audience] 料可讓您更精細地瞭解您打算傳送至目的地的細分，以便鎖定受眾並啟用。
...
所有日期和日期範圍皆設定在時 [!DNL UTC] 區中。 請參 [閱Audience manager中的時區](../reference/aam-time-zones.md)。
...
此量度會傳回所有Audience manager客戶所收集之活動的資料。 相較於匯整的Audience manager客戶，他們可以提供更廣泛的客戶觀眾。
...
<table id="table_895D536F69134330A4F13887ECAFD4F5"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>大部份的伺服器對伺服器整合都仰賴協力廠商Cookie所協助的同步程式。 不過，行動環境不會使用協力廠商Cookie。 因此，與區段大小相比，您的「可定址對象」數目看起來可能較低。 </p> <p>自2018年1月起，您可以在為Cookie型觀眾設定的相同Google和Adobe Advertising cloud目的地中啟用行動觀眾。 雖然這表示您可以將結合了Cookie和行動ID會籍的區段傳送至您的Google和Advertising cloud目標，但請記住「可定址對象」只會顯示Cookie ID和目標之間的重疊。 Audience manager會傳送100%的行動對象至目標，但行動對象不會由「可定址對象」量度測量。 </p> <p> <p><b>注意</b>: 例如，以人口為1,000,000的區段為例。 如果您將此區段對應至Google或Adobe Advertising cloud目標，您可能會看到700,000個裝置的可定址對象，以及70%的符合率。 700,000個會籍包含Cookie ID，這些ID與目標同步。 實際上，您的可定址對象可能會更高，因為可定址行動ID不會顯示在此量度中。 </p> </p> </td> 
  </tr> 
  <tr> 
...
例如，假設您傳送區段至目標，而該目標顯示低報告數。 檢查結 [!UICONTROL Addressable Audience] 果會顯示這是技術問題，還是只是低比對率的問題。 低比對率顯示您的目的地對您選取的區段並不太適合。 不過，Audience manager與目標之間可定址觀眾總數的差異，表示整合、同步或其他技術問題。 在這些情況下，請連絡您的客戶經理。
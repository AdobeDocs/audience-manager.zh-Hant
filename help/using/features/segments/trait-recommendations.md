...
從觀看下面的[!UICONTROL Trait Recommendations]影片開始，然後閱讀以取得更多資訊。 影片示範會示範如何使用您自己的第一方特徵建議，以及[!UICONTROL Audience Marketplace]您已訂閱&#x200B;*之*&#x200B;資料摘要的特徵建議。
...
[!UICONTROL Trait Recommendations]由[!DNL Adobe Sensei]提供技術支援，將資料科學帶入您的Audience Manager日常工作流程。
...
![特徵建議總覽](assets/trait-recommendations-overview-full.png)
...
* Audience Manager最多會顯示五十個與區段規則類似的特徵。
* 您可以篩選掉不想看到任何建議的資料來源。
...
透過[!UICONTROL Trait Recommendations]，您可以根據使用Audience Manager的方式，改善工作流程：
...
1. 您可以在[!UICONTROL Audience Marketplace]區段中檢視您所訂閱之摘要的第一方建議特徵和&#x200B;**[!UICONTROL Recommendations]**&#x200B;特徵建議。 **[!UICONTROL Recommendations from Marketplace]**&#x200B;區段會顯示您未訂閱之摘要的特徵建議。 所有這些建議與新增至區段規則的特徵類似。 向下捲動以檢視所有建議的特徵。
...
為了產生特徵建議，Audience Manager會計算目標特徵與您的帳戶可存取的所有其他特徵（包括第三方資料）之間的[Jaccard相似度](https://en.wikipedia.org/wiki/Jaccard_index)。 接著Audience Manager會顯示最多50個相似度最高的特徵。
...
在[!UICONTROL RBAC]此處[閱讀有關](../administration/administration-overview.md)控制項的詳細資訊。

## 限制

* 目前，Audience Manager不會將資料夾特徵顯示為建議的特徵。 在[這裡](../traits/manage-folder-traits.md)閱讀更多有關資料夾特徵的資訊。
* 顯示特徵建議時，Audience Manager在區段規則中未考慮[!DNL Boolean]運運算元([!DNL AND]、[!DNL OR]、[!DNL NOT])。

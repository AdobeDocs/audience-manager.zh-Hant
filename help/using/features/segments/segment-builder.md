...
1. 在[基本資訊](../../features/segments/segment-builder.md#segment-builder-controls-basics)區段中：
   ...
   * 選取要用於[!UICONTROL profile merge rule]資格的[!UICONTROL segment]。
...
1. 在[特徵](../../features/segments/segment-builder.md#segment-builder-controls-traits)區段中：
   ...
   * 按一下[!UICONTROL Advanced Search]以開啟&#x200B;**[!UICONTROL Browse All Traits]**&#x200B;強制回應視窗。 依名稱、識別碼、說明或[!UICONTROL traits]搜尋[!UICONTROL data source]。 在搜尋時按一下資料夾，將結果限制在該資料夾及其子資料夾中。 您也可以依[!UICONTROL traits] （[!UICONTROL trait type]、[!UICONTROL Folder Trait]、[!UICONTROL Rule-based]和[!UICONTROL Onboarded]）或母體型別（[!UICONTROL Algorithmic]裝置ID[和](../../reference/ids-in-aam.md)跨裝置ID[）篩選](../../reference/ids-in-aam.md)。
     ![segment-builder-browser-traits](assets/segment-builder-browse-traits.png)
   * 在您建置[時取得即時](trait-recommendations.md)特徵建議[!UICONTROL segment]。
   ...
   * 新增或移除[!UICONTROL traits]時檢視區段母體資料。 按一下&#x200B;**[!UICONTROL Calculate Estimates]**&#x200B;以檢視（或重新整理）預估的母體數目。 深入瞭解[中的](../../features/segments/segment-builder-data.md#segment-populations)區段母體資料[!UICONTROL Segment Builder]。
   * 完成時，按一下&#x200B;**[!UICONTROL Save]**。

1. *（選擇性）*&#x200B;將[!UICONTROL segment]對應到[!UICONTROL destination]目的地對應[區段中的](../../features/segments/segment-builder.md#segment-builder-controls-destinations)：
...
| **[!UICONTROL Data Source]** | 將區段與特定資料提供者建立關聯。 <br>使用第一個下拉式功能表，在Audience Manager資料來源、Adobe Analytics報表套裝或兩者之間篩選。 然後，使用第二個下拉式選單來選擇資料來源。 <br>如果您未使用Adobe Analytics報表套裝，資料來源型別選擇器會停用，且僅預設為Audience Manager資料來源。 |
...
在[!UICONTROL Segment Builder]中，[!UICONTROL Traits]區段可讓您管理[!UICONTROL traits]中的[!UICONTROL segment]、建立[!UICONTROL trait]群組，以及設定資格條件。 若要將[!UICONTROL trait]新增至[!UICONTROL segment]，請在搜尋欄位中輸入[!UICONTROL trait]名稱，然後按一下[!UICONTROL Add Trait]。 儲存[!UICONTROL trait] （如果完成）或移至[!UICONTROL Destinations Mapping]。
...
| 欄位 | 說明 |
|--- |--- |
| **[!UICONTROL Basic View]** | 本節提供視覺化控制項，可讓您： <ul><li>建置新專案並管理現有的[!UICONTROL segments]。</li><li>從[!UICONTROL traits]移除[!UICONTROL segment]。</li><li>將最多50個（最多） [!UICONTROL traits]新增至[!UICONTROL segment]。</li><li>拖放[!UICONTROL traits]以建立新群組。</li><li>在[!UICONTROL traits]中檢視[!UICONTROL trait]和[!UICONTROL segment]群組。</li><li>使用布林運算式、比較運運算元和時近/頻率設定來設定資格條件。</li></ul> |
| **[!UICONTROL Code View]** | 開啟開發環境，讓您使用程式碼而非視覺化介面來建立和管理[!UICONTROL traits]、群組和資格需求。 若您的[!UICONTROL segments]： <ul><li>在個別[!UICONTROL traits]中包含超過50個[!UICONTROL segment]。 注意： [!UICONTROL Segments]限製為5000 [!UICONTROL traits] （最大值）。</li><li>包含許多[!UICONTROL trait]群組。</li><li>具有複雜的資格要求。</li></ul> |
| 搜尋 | 協助您尋找[!UICONTROL traits]以新增至[!UICONTROL segment]。 |
| 建議 | 從您訂閱的第一方[!UICONTROL traits]和[!UICONTROL traits]資料摘要，取得類似[!UICONTROL Audience Marketplace]的即時建議。 將這些建議新增至[!UICONTROL segment]規則以擴大您的對象。 請參閱[特徵建議](trait-recommendations.md)以瞭解詳情。 |
| **[!UICONTROL Marketplace Recommendations]** | 從您未訂閱的[!UICONTROL traits]資料摘要取得類似[!UICONTROL Audience Marketplace]的即時建議。 請參閱[特徵建議](trait-recommendations.md)以瞭解詳情。 |
| 實際和預估的[!UICONTROL Segment]大小資料 | 請參閱[區段產生器的特徵和區段母體資料](segment-builder-data.md)。 |

## 從[!UICONTROL Traits]移除[!UICONTROL Segment] {#remove-traits}

管理您[!UICONTROL traits]中的[!UICONTROL segments]是保持[!UICONTROL segments]可行性的重要部分。 如果您需要從[!UICONTROL traits]移除[!UICONTROL segment]，請依照下列步驟進行。

若要從[!UICONTROL traits]移除[!UICONTROL segment]：
...
4. 將游標暫留在您要刪除的[!UICONTROL trait]上，然後按一下X。此動作會立即從您的[!UICONTROL trait]移除[!UICONTROL segment]。
...
在[!UICONTROL Segment Builder]中，在您選取[!UICONTROL Add Destination]之後，[!UICONTROL destination]對話方塊就會顯示。 此視窗顯示有關[!UICONTROL destination]的靜態資訊，以及根據[!UICONTROL destination]型別而變化的欄位。 在空白欄位中提供設定[!UICONTROL destination mapping]的必要資訊。

...
1. **[!UICONTROL Segment Graph]：**&#x200B;以圖形方式顯示效能資料，固定間隔為1、7、14、30、60和90天。 我們在[另一篇文章](../../features/segments/segment-builder-data.md)中說明區段母體數目。

   ![區段圖表](assets/segment-graph.png)

1. **[!UICONTROL Identity Type Breakdown]：**&#x200B;報表會計算連結至符合區段資格之裝置的跨裝置ID和/或外部裝置圖表ID數量，藉此顯示符合區段資格之人員或家庭數量（由[!UICONTROL Total Segment Population]顯示）。 此報表中顯示的跨裝置ID和外部裝置圖表ID，可用來將設定檔與區段正在使用的設定檔合併規則合併。 此報表僅當您在區段使用的設定檔合併規則中選取跨裝置資料來源或外部裝置圖表時才會顯示。
...
1. **[!UICONTROL Segment Rules]：**&#x200B;列出區段中的特徵以及資格規則。

...
>
> 希望繼續使用Audience Manager Plug-in for IAB TCF進行許可管理的客戶，應升級至最新版[ECID](https://github.com/Adobe-Marketing-Cloud/id-service/releases)以取得持續支援。
>
...
>[!WARNING]
>
>如果您使用不支援IAB TCF v.2.0的「同意管理平台」(CMP),Audience Manager會自動傳送ID同步中的`gdpr=0`參數，即使您的訪客是歐盟地區的訪客亦然。 要確定您的GDPR驗證是否處於活動狀態，建議您向許可管理平台(CMP)確認它們支援IAB TCF v2.0。
...
Audience Manager會根據[IAB歐洲透明度與同意框架政策](https://iabeurope.eu/iab-europe-transparency-consent-framework-policies/#A_Purposes)中定義的下列目的，評估儲存在IAB TC字串中的使用者選擇。 此外，您也可以在[全域廠商清單](https://vendorlist.consensu.org/vendorlist.json)中找到相關用途。
...
6. Audience Manager會在瀏覽器上設定[demdex Cookie](https://docs.adobe.com/content/help/zh-Hant/core-services/interface/ec-cookies/cookies-am.translate.html)來回應，並啟動並接受協力廠商ID同步。
...
4. 如果IAB TC字串遺失或缺乏必要的權限，Audience Manager會在像素呼叫中刪除在`gdpr`和`gdpr_consent`變數中傳遞的資料。
...
與IAB TCF v2.0整合的Audience Manager支援附加同意書至傳送至與IAB TCF v2.0整合之[URL目標](../../features/destinations/create-url-destination.md)的資訊。不過，Audience Manager不會自動執行此程式，以避免中斷特定URL格式。
...
當您的網站訪客未提供適當的權限時，IAB TCF的Audience Manager外掛程式會自動選出請求中顯示的ID。 如果請求包含[跨裝置ID(CRM ID)](../../reference/ids-in-aam.md),Audience Manager會選取該ID，以及連結至該[跨裝置ID(CRM ID)](../../reference/ids-in-aam.md)的最後一個裝置。

## 測試您的 IAB 實作 {#test-iab-implementation}

若要測試您是否已正確實作IAB TCF的Audience Manager外掛程式，請閱讀[驗證選擇加入服務的使用案例4](https://docs.adobe.com/content/help/zh-Hant/id-service/using/implementation/opt-in-service/testing-optin-and-iab-plugin.translate.html#section-64331998954d4892960dcecd744a6d88)。

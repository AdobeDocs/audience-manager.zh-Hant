...
>[!IMPORTANT]
>
>Audience Manager已註冊於 [IAB TCF](https://iabeurope.eu/tcf-for-vendors/) 供應商ID為565。
...
升級客戶 [!DNL Audience Manager Plug-in for IAB TCF] 實施自 [!DNL IAB TCF] v1.1至 [!DNL IAB TCF] v2.0，或啟用 [!DNL IAB TCF] 第一次使用v2.0時，所有使用者在必要條件和實施方面都應遵循下列相同准則。
...
>
> 若客戶希望繼續使用適用於IAB TCF的Audience Manager外掛程式來管理同意，請升級至的最新版本 [ECID](https://github.com/Adobe-Marketing-Cloud/id-service/releases) 以繼續支援。
>
> 升級至最新 [ECID](https://github.com/Adobe-Marketing-Cloud/id-service/releases) 版本中不再支援IAB TCF v1.1同意字串，因此請務必更新CMP，再升級至最新ECID版本。
...
2. 您必須使用Audience Manager [!DNL Data Integration Library] (DIL)9.0版或更新版本，可從 [此處](https://github.com/Adobe-Marketing-Cloud/dil/releases). 閱讀 [Audience Manager 中的 DIL 文件](../../dil/dil-overview.md)。建議您使用 [Adobe Audience Manager標籤擴充功能](https://experienceleague.adobe.com/docs/experience-platform/tags/extensions/adobe/audience-manager/overview.html) 以最輕鬆的DIL實作Audience Manager。
3. 或者，如果您使用 [!DNL Server-Side Forwarding] (SSF)若要將資料匯入Audience Manager，您必須升級至最新版的AppMeasurement。 使用 [Analytics 代碼管理器](https://experienceleague.adobe.com/docs/analytics/admin/admin-tools/code-manager-admin.html)下載 AppMeasurement。
4. 您必須使用與IAB TCF v2.0整合且已向IAB TCF註冊的同意管理平台(CMP)，商業版或自有版皆可。 請參閱[在 IAB 框架內註冊的 CMP](https://iabeurope.eu/cmp-list/) 清單。

>[!WARNING]
>
>如果您使用不支援IAB TCF v.2.0的同意管理平台(CMP),Audience Manager會自動傳送 `gdpr=0` 參數，即使您的訪客是歐盟的。 若要判斷您的GDPR驗證是否有效，建議您向同意管理平台(CMP)確認其支援IAB TCF v2.0。
...
最簡單的方式是使用 [Adobe Experience Platform標籤](https://experienceleague.adobe.com/docs/experience-platform/tags/home.html?lang=en) 新增 [!DNL ECID Opt-in] 在屬性上。 請參閱 [ECID選擇加入擴充功能](https://experienceleague.adobe.com/docs/experience-platform/tags/extensions/adobe/id-service/overview.html) 了解如何設定「標籤」擴充功能。

## 使用 IAB 架構時的使用者選擇工作流程 {#user-choice-workflow}

造訪 Web 屬性時，您的使用者可以選擇提供其資料供發佈商及與發佈商合作的第三方廠商使用。

使用者提供其選擇的形式為 *同意* 和 *合法利益* IAB用途 *協力廠商* 已在全球供應商清單中註冊。

下圖呈現 CMP 對話方塊的範例，向首度造訪網站的訪客顯示。請記得，此對話方塊的外觀會因為客戶實作不同而有很大差異。

![CMP 對話框](assets/cmp-example.png)

有關IAB TCF v2.0中所包含各種用途和權限的詳細資訊，請參閱 [IAB歐洲透明與同意框架政策](https://iabeurope.eu/iab-europe-transparency-consent-framework-policies/#A_Purposes).
...
使用者選取其隱私權選擇後，使用者選擇便會記錄在IAB TC字串中。 IAB TC字串會儲存已核准用途與廠商的組合，以及其他中繼資料資訊(請參閱 [IAB頁面](https://github.com/InteractiveAdvertisingBureau/GDPR-Transparency-and-Consent-Framework/blob/master/TCFv2/IAB%20Tech%20Lab%20-%20Consent%20string%20and%20vendor%20list%20formats%20v2.md#about-the-transparency--consent-string-tc-string) 以取得詳細資訊)。

在IAB TCF中註冊的每家廠商都會評估IAB TC字串，並根據使用者的隱私權選擇做出決策。 請記得，使用者的隱私權選擇在所有透過IAB TCF註冊的廠商中都有效。

## 所需用途(按Audience Manager) {#aam-standard-purposes}

Audience Manager會針對下列用途評估儲存在IAB TC字串中的使用者選擇： [IAB歐洲透明與同意框架政策](https://iabeurope.eu/iab-europe-transparency-consent-framework-policies/#Appendix_A_Purposes_and_Features_Definitions).
...
>
>Per [IAB法規](https://iabeurope.eu/iab-europe-transparency-consent-framework-policies/#Special_Purpose_1__Ensure_security_prevent_fraud_and_debug_)、特殊用途1（確保安全、防止欺詐和除錯）一律同意，且使用者無法反對。
...
|---|---|
| <ul><li>會執行您請求的所有 Audience Manager 使用案例。</li><li>以ID同步傳達對第三方的同意(透過 `gdpr = 1` 同意字串為 `gdpr_consent` （在ID同步呼叫上）。</li><li>會評估並遵照從廣告伺服器像素傳遞的同意。</li><li>會執行合作夥伴起始的 ID 同步。</li></ul> | <ul><li>不會在您的例項中儲存任何新使用者資料。這包括合作夥伴 ID、訊號、特徵或像素資料。</li><li>不會起始第三方 ID 同步。</li><li>不會執行合作夥伴起始的 ID 同步。</li><li>將使用者退出進一步的資料收集。</li></ul> |
...
3. Audience Manager檢查GDPR是否適用(`gdpr = 1`)，以及您的Web屬性上是否有已向IAB TCF註冊的CMP。 例如，這適用於來自歐盟的造訪使用者。 請注意，您身為發佈商，有責任設定GDPR標幟。
4. 若GDPR適用，Audience Manager會檢查傳入 `gdpr_consent` 參數，以取得必要的同意。 Audience Manager需要儲存和/或存取裝置上資訊的同意([IAB TCF用途1](https://iabeurope.eu/iab-europe-transparency-consent-framework-policies/#A_Purposes))、開發及改進產品([IAB TCF用途10](https://iabeurope.eu/iab-europe-transparency-consent-framework-policies/#A_Purposes))，加上Audience Manager廠商同意儲存、處理或啟用資料。
...
6. Audience Manager透過設定 [demdex cookie](https://experienceleague.adobe.com/docs/core-services/interface/ec-cookies/cookies-am.html) 在瀏覽器上，啟動並執行協力廠商ID同步。
7. 或者，如果在步驟4傳遞的IAB TC字串不包含所有需要的權限，Audience Manager就不會收集、處理或啟用任何使用者資料，也不會執行或起始ID同步。 此外，它也會從您使用的目的地中選擇使用者。

>[!IMPORTANT]
>
>如果您與需要IAB TCF參數的Audience Manager目的地合作夥伴合作，但您的網站上沒有支援IAB TCF的CMP，則Audience Manager會傳送 `gdpr=0` 在ID同步中。 這表示GDPR不適用於這些使用者。
>
...
1. 系統會透過廣告伺服器向您的使用者顯示一次曝光內容。這會轉換為 [像素呼叫](../../integration/media-data-integration/impression-data-pixels.md) 資料收集伺服器(DCS)。
2. Audience Manager 會檢查 GDPR 標幟是否適用。如果沒有，Audience Manager會儲存傳入 `gdpr` 和 `gdpr_consent` 像素呼叫中的變數。
3. 如果IAB TC字串存在且包含必要的權限，Audience Manager會儲存傳入 `gdpr` 和 `gdpr_consent` 像素呼叫中的變數。
4. 如果IAB TC字串遺失或缺少必要的權限，Audience Manager會捨棄傳入 `gdpr` 和 `gdpr_consent` 像素呼叫中的變數。
...
與IAB TCF v2.0的Audience Manager整合支援將同意附加至傳送至的資訊 [URL目的地](../../features/destinations/create-url-destination.md) 與IAB TCF v2.0整合。不過，此程式不會由Audience Manager自動完成，以避免中斷特定URL格式。

想要將同意附加至傳送至的資料的客戶 [!DNL URL destinations] 必須手動新增 `${GDPR}` 和 `${GDPR_CONSENT_XXXX}` 巨集轉換為其URL格式，取代 `XXXX` 搭配目的地合作夥伴ID。

範例: `https://yourdomain.com?gdpr=${GDPR}&gdpr_consent=${GDPR_CONSENT_1234}`.

請參閱 [定義的巨集目標](../../features/destinations/destination-macros.md) 有關支援的目標宏的詳細資訊。

## 跨裝置同意管理

適用於IAB TCF的Audience Manager外掛程式會在您的網站訪客未提供適當權限時，自動選出請求上顯示的ID。 如果請求包含 [跨裝置ID(CRM ID)](../../reference/ids-in-aam.md),Audience Manager會選出ID，以及連結至該ID的最後一個裝置 [跨裝置ID(CRM ID)](../../reference/ids-in-aam.md).

## 測試您的 IAB 實作 {#test-iab-implementation}

若要測試您是否已正確實作適用於IAB TCF的Audience Manager外掛程式，請參閱 [驗證選擇加入服務的使用案例4](https://experienceleague.adobe.com/docs/id-service/using/implementation/opt-in-service/testing-optin-and-iab-plugin.html#section-64331998954d4892960dcecd744a6d88).
